...
import { LockIcon, ArrowUpIcon, ActivityIcon, ExternalLinkIcon, ChevronDownIcon, ChevronUpIcon } from '@/components/ui/Icons';

const TYPE_CONFIG = {
  deposit:     { Icon: LockIcon,     label: 'Locked',       color: 'var(--gold)'  },
  withdraw:    { Icon: ArrowUpIcon,  label: 'Withdrawn',    color: 'var(--green)' },
  withdrawAll: { Icon: ArrowUpIcon,  label: 'Withdraw All', color: 'var(--green)' },
...
function TxRow({ event, explorerBase }) {
  ...
  return (
    <div className="tx-row">
      <div className="tx-icon-wrap" style={{ background: `color-mix(in srgb, ${cfg.color} 12%, transparent)` }}>
      ...
      <div className="tx-body">
        <div className="tx-label">
          ...
          {event.vaultId != null && (
            <span className="vault-id-badge">#{String(event.vaultId).padStart(3, '0')}</span>
          )}
        </div>
...
export default function TransactionHistory({ history, isLoading, explorerBase = 'https://sepolia.etherscan.io' }) {
  const [filter,   setFilter]   = useState('all');
  const [expanded, setExpanded] = useState(false);

  const filtered = history.filter(e =>
    filter === 'all'     ? true :
    filter === 'deposit' ? e.type === 'deposit' :
    ...
    <div className="card card-accent-blue" style={{ marginTop: 0 }}>
      <div className="card-title" style={{ justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 3 }}>
          {FILTERS.map(f => (
            <button key={f.id} className={`tab ${filter === f.id ? 'active' : ''}`}
      ...
      {isLoading ? (
        ...
        <div className="empty-state">
          <div className="empty-icon-wrap"><ActivityIcon size={20} color="var(--text-muted)" /></div>
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {visible.map(e => <TxRow key={e.id} event={e} explorerBase={explorerBase} />)}
          </div>
          {filtered.length > 5 && (
            <button className="btn btn-secondary btn-full" style={{ marginTop: 10 }}
              ...
              {expanded
                ...
                : <><ChevronDownIcon size={13} /> Show All {filtered.length} Transactions</>
              }
            </button>
          )}
        </>
