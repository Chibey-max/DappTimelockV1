  ...
  return (
    <div className="card" style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
        <div className="card-title" style={{ marginBottom: 0 }}>Portfolio History</div>
        <div style={{ textAlign: 'right' }}>
          ...
          {ethPrice && (
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>
              ${(currentBalance * ethPrice).toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
          {isUp ? '+' : ''}{pnlEth.toFixed(4)} Ξ all time
        </span>
      </div>
      ...
      <div style={{
        ...
        {[
          { label: `Aave (~${(AAVE_APY*100).toFixed(1)}% APY)`, value: aaveYield, color: '#3b82f6' },
          { label: `stETH (~${(STETH_APY*100).toFixed(1)}% APY)`, value: stethYield, color: '#8b5cf6' },
          ...
          <div key={label} style={{
            ...
            <div style={{ fontSize: '0.95rem', fontWeight: 700, color, fontFamily: 'Syne, sans-serif' }}>
              +{value.toFixed(5)} Ξ
            </div>
